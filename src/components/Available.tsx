...
export default function Available({
    ...
    return ( 
        <div className="w-[100%] h-[35vh] border-4 border-bg-black flex flex-col space-y-10 justify-center rounded-2xl m-5 items-center">
          ...
          {isDisabled ? (
                <button className="bg-gray-400 text-white py-5 px-3 rounded-lg relative text-center text-xl cursor-not-allowed" disabled>
                    You can reserve only 3 reservation
                </button>
            ) : (
                <Link href={`/booking/?id=${id}`} className="w-[95%] flex justify-center items-center">
                    <button
